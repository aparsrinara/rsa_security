...
static void generate_prime(mpz_t p, unsigned int numbits)
{
	...
	while (1) {
		array = malloc(sizeof(char*)*(numbits/8));
		fread(array, 1, numbits/8, f);
		array[0] = array[0] | 0xc0;
		mpz_import(p, numbits/8, 1, 1, 0, 0, array);
		if (mpz_probab_prime_p(p, 25) >= 1) {
			break;
		}
		free(array);
	}
...
void rsa_genkey(struct rsa_key *key, unsigned int numbits)
{
	...
	char* c = "1";
	mpz_t d;
	mpz_init_set_str(d, c, 10);
	mpz_sub(b, p, d);
	mpz_sub(a, q, d);
	mpz_mul(i, a, b);
	mpz_invert(key->d, key->e, i);
}